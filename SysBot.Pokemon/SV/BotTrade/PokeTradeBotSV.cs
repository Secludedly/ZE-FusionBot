...
public class PokeTradeBotSV(PokeTradeHub<PK9> Hub, PokeBotState Config) : PokeRoutineExecutor9SV(Config), ICountBot, ITradeBot
{
    ...
    private void HandleAbortedTrade(PokeTradeDetail<PK9> detail, PokeRoutineType type, uint priority, PokeTradeResult result)
    {
        ...
        if (result.ShouldAttemptRetry() && detail.Type != PokeTradeType.Random && !detail.IsRetry)
        {
            ...
            detail.SendNotification(this, "*An error occurred.*\n**Status:** Requeuing");
        }
        else
        {
            detail.SendNotification(this, $"*An error occurred.*\n**Status:** Canceling\n**Reason:** {result}.");
    ...
    private async Task<PokeTradeResult> PerformNonBatchTrade(SAV9SV sav, PokeTradeDetail<PK9> poke, CancellationToken token)
    {
        ...
        Log($"Found a User! OT: {tradePartner.TrainerName} // TID: {tradePartner.TID7} // SID: {tradePartner.SID7} // NID: {trainerNID}");
        ...
        var gameVersion = GameVersion.SV; // Or set this to the appropriate game version based on context
    ...
    private async Task<(PK9 toSend, PokeTradeResult check)> HandleClone(SAV9SV sav, PokeTradeDetail<PK9> poke, PK9 offered, byte[] oldEC, CancellationToken token)
    {
        ...
        if (!la.Valid)
        {
            Log($"Invalid Pokémon: {GameInfo.GetStrings(1).Species[offered.Species]}.");
            if (DumpSetting.Dump)
                DumpPokemon(DumpSetting.DumpFolder, "hacked", offered);

            var report = la.Report();
            Log(report);
            poke.SendNotification(this, "**Error:** Pokémon is not legal.\n**Result:** Exiting trade.");
        ...
        poke.SendNotification(this, $"**Cloned:** {GameInfo.GetStrings(1).Species[clone.Species]}!**\n**1.** Press B to cancel offer.\n**2.** Trade a Pokémon you don't want.");
        Log($"**Cloned:** {GameInfo.GetStrings(1).Species[clone.Species]}.\n*Waiting for Pokémon...*");
        ...
        if (!partnerFound)
        {
            poke.SendNotification(this, "**Change it now or I will cancel.**");
