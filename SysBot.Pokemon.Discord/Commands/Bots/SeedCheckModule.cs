...
public class SeedCheckModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
{
    ...
    public async Task SeedCheckAsync([Summary("Trade Code")][Remainder] string code)
    {
        ...
        var sig = Context.User.GetFavor();
        await QueueHelper<T>.AddToQueueAsync(Context, tradeCode == 0 ? Info.GetRandomTradeCode(userID, Context.Channel, Context.User) : tradeCode, Context.User.Username, sig, new T(), PokeRoutineType.SeedCheck, PokeTradeType.Seed).ConfigureAwait(false);
    }
