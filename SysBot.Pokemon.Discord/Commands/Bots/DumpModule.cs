...
public class DumpModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
{
    ...
    public async Task DumpAsync([Summary("Trade Code")][Remainder] string code)
    {
        ...
        var sig = Context.User.GetFavor();
        await QueueHelper<T>.AddToQueueAsync(Context, tradeCode == 0 ? Info.GetRandomTradeCode(Context.User.Id, Context.Channel, Context.User) : tradeCode, Context.User.Username, sig, new T(), PokeRoutineType.Dump, PokeTradeType.Dump);
    }
